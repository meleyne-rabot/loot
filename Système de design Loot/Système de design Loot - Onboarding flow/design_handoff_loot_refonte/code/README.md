...
- `DetailArticle.jsx` — **détail d'un article**, plein écran, onglets **Suivi & prix** (défaut : hero navy marge + reco IA, prix achat/listé/vente réel, statut, plateforme, source, malle, suppression) / **Annonce** (blocs Copier titre/description + hashtags Vinted, régénérer, tags). Champ « Note prix » supprimé.
- `Faq.jsx` — **écran FAQ** (ouvert depuis le menu profil) : header croix, carte navy d'intro, accordéon (1re ouverte) avec réponses rédigées, bloc contact. Props : `onClose`, `onContact`.
- `Onboarding.jsx` — **carousel de bienvenue** (5 slides : Bienvenue navy → Photo → IA/copier → Gains → Malles). Points de progression, « Passer », swipe tactile, CTA final. Exports `hasSeenOnboarding()` / `markOnboardingSeen()` pour l'afficher **au 1er lancement seulement** ; monter `<Onboarding replay onDone={…} />` depuis **Aide** pour le rejouer.
